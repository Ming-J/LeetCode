...
public:
  double minAreaFreeRect(vector<vector<int>> &points) {
    unordered_map<string, vector<vector<vector<int>>>> midpoints;
    ...
    for (auto comb : res) {
      ...
      string mid_str =
          to_string(distanceSqr(comb[0], comb[1])) + ' ' + pointToString(midp);
      if (midpoints.find(mid_str) == midpoints.end())
        midpoints[mid_str].push_back({comb[0], comb[1]});
      else {
        for (auto diagonal : midpoints[mid_str]) {
          area = min(area, calArea(midp, diagonal, comb[0], comb[1]));
        }
        midpoints[mid_str].push_back({comb[0], comb[1]});
        //cout << mid_str << " p1: " << pointToString(comb[0])
        //     << " p2: " << pointToString(comb[1]) << endl;
        // area = min(area, calArea(midp, midpoints[mid_str], comb[0],
        // comb[1]));
      }
    }
