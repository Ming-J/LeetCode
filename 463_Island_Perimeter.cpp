...
public:
  ...
  void dsf(vector<vector<int> >& grid, int &primeter, int row, int col,
	...
	return;
      ...
      if(row+1<rowUB&&grid[row+1][col]==0){primeter++;}
      if(row-1>=0&&grid[row-1][col]==0){primeter++;}
      if(col+1<colUB&&grid[row][col+1]==0){primeter++;}
      if(col-1>=0&&grid[row][col-1]==0){primeter++;}
